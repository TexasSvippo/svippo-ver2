                      ...
                      <div key={order.id} className={`${styles.placed_card} ${borderCls}`} onClick={() => router.push(`/order/${order.id}`)} style={{ cursor: 'pointer' }}>
                        ...
                        <div className={styles.placed_card__header}>
                          <div className={styles.placed_card__header_left}>
                            ...
                            <span className={styles.placed_card__buyer_name}>{order.buyer_name}</span>
                          </div>
            ...
            <div className={styles.profile__section}>
              ...
              {placedOrders.length === 0 ? (
                ...
                const renderPlacedCard = (order: PlacedOrder) => {
                  ...
                  const ps = order.project_status
                  const borderCls = (ps === 'cancelled' || order.status === 'cancelled') ? styles['placed_card--cancelled'] : (ps === 'awaiting_confirmation' || ps === 'delivered') ? styles['placed_card--action'] : ps === 'completed' || order.status === 'rejected' ? styles['placed_card--done'] : styles['placed_card--ongoing']
                  const friendlyStatus = (ps === 'cancelled' || order.status === 'cancelled') ? 'Avbokad' : order.status === 'rejected' ? 'Nekad' : ps === 'completed' ? 'Avslutat' : ps === 'awaiting_confirmation' ? 'Inväntar din bekräftelse' : ps === 'delivered' ? 'Inväntar godkännande' : order.status === 'accepted' ? 'Pågår' : 'Väntar på utföraren'
                  const statusCls = (ps === 'cancelled' || order.status === 'cancelled') ? styles['placed_card__status_text--cancelled'] : (ps === 'awaiting_confirmation' || ps === 'delivered') ? styles['placed_card__status_text--action'] : ps === 'completed' || order.status === 'rejected' ? styles['placed_card__status_text--done'] : styles['placed_card__status_text--ongoing']
                  const stepKeys = ['pending', 'in_progress', 'delivered', 'completed']
                  const stepLabels = ['Beställd', 'Pågår', 'Levererat', 'Avslutat']
                  const currentStep = ps === 'completed' ? 3 : (ps === 'awaiting_confirmation' || ps === 'delivered') ? 2 : (ps === 'in_progress' || ps === 'almost_done') ? 1 : order.status === 'accepted' ? 1 : 0
                  ...
                  return (
                    <div key={order.id} className={`${styles.placed_card} ${borderCls}`} onClick={() => router.push(`/my-order/${order.id}`)} style={{ cursor: 'pointer' }}>
                      ...
                      {(() => {
                        ...
                        if (ps === 'awaiting_confirmation') return <div className={styles.placed_card__actions}><Link href={`/my-order/${order.id}`} className={styles.placed_card__btn_primary} onClick={e => e.stopPropagation()}>Bekräfta slutfört →</Link><button type="button" className={styles.placed_card__btn_ghost} onClick={openChat}>Öppna chatt</button></div>
                        ...
                        if (order.status === 'accepted' && ps !== 'completed' && ps !== 'delivered' && ps !== 'awaiting_confirmation') return <div className={styles.placed_card__actions}><button type="button" className={styles.placed_card__btn_ghost} onClick={openChat}>Öppna chatt</button><button type="button" className={styles.placed_card__btn_ghost} onClick={e => { e.stopPropagation(); router.push(`/my-order/${order.id}`) }}>Visa detaljer</button></div>
