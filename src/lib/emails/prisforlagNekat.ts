...
type SendPrisforlagNekatParams = {
  to: string
  sellerName: string
  buyerName: string
  orderUrl: string
...
export async function sendPrisforlagNekat({
  to,
  sellerName,
  buyerName,
  orderUrl,
}: SendPrisforlagNekatParams) {
  const html = `
<!DOCTYPE html>
<html lang="sv">
          ...
          <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;">
            ...
            <tr>
              <td style="padding:32px 24px;">
                <h1 style="margin:0 0 16px;font-size:22px;line-height:1.3;color:#1a1a1a;">Ditt prisförslag avböjdes</h1>
                <p style="margin:0 0 24px;font-size:15px;line-height:1.6;color:#444444;">Hej ${escapeHtml(sellerName)}! ${escapeHtml(buyerName)} har tyvärr avböjt ditt prisförslag. Du kan skicka ett nytt förslag om du vill.</p>
                ...
                <p style="margin:24px 0 0;font-size:13px;line-height:1.5;color:#888888;text-align:center;">Gå in på beställningen och skicka ett reviderat förslag om du vill.</p>
              </td>
            </tr>
            <tr>
  ...
  return sendEmail({
    to,
    subject: 'Ditt prisförslag avböjdes',
    html,
  })
}
